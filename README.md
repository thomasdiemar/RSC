...
## Projects
- `RCS` – core domain types (vectors, thrusters, engine, command) plus the generic `ARcsEngineOptimiser` that builds the coefficient matrix and desired vector from a fleet of thrusters.
...
## How it works
1. `ARcsEngineOptimiser` orders thrusters, builds a 6×N coefficient matrix (Fx/Fy/Fz/Tx/Ty/Tz rows), and derives desired row targets from an `RcsCommand`.
